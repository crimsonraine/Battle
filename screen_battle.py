...
class Screen_Battle (tk.Frame):
    ...
    def create_widgets (self):
        ...
        self.atk_bttn.grid(row = 0, column = 0)

        # three lines of labels for move announcements
        self.annc_atk = tk.Label(self) 
        self.annc_atk.grid(row = 0, column = 1, sticky = tk.N) 
        self.annc_atk2 = tk.Label(self)
        self.annc_atk2.grid(row = 1, column = 1, sticky = tk.N)
        self.annc_atk3 = tk.Label(self) 
        self.annc_atk3.grid(row = 2, column = 1, sticky = tk.N) 
        ...
        # HP labels
        self.player_hp_lbl = tk.Label(self, text = f"{self.player1.hit_points}/{self.player1_max_hp} HP")
        self.player_hp_lbl.grid(row = 5, column = 0, sticky = tk.N)

        self.computer_hp_lbl = tk.Label(self, text = f"{self.player2.hit_points}/{self.player2_max_hp} HP")
        self.computer_hp_lbl.grid(row = 5, column = 1, sticky = tk.N)

        self.exit_bttn = tk.Button(self, text = "Exit", bg = 'black', fg = 'red')
        self.exit_bttn["command"] = self.exit_clicked
        
    def attack_clicked(self):
        self.annc_atk['text'] = str(self.player1.attack(self.player2))
        
        if self.player2.hit_points > 0: 
            self.annc_atk2['text'] = self.player2.attack(self.player1)
        else: # computer dead
            self.annc_atk3['text'] = self.player1.name + " wins!"
            self.atk_bttn.destroy() 
            self.exit_bttn.grid(row = 6, column = 1, pady = 5, sticky = tk.S + tk.E)

        if self.player1.hit_points <= 0: # player dead
            self.annc_atk3['text'] = self.player2.name + " wins!"
            self.atk_bttn.destroy() 
            self.exit_bttn.grid(row = 6, column = 1, pady = 5, sticky = tk.S + tk.E)
